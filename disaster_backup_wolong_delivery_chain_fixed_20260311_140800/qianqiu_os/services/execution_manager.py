...
class ExecutionManager:
    def attach_delivery_payload(self, execution_task, formatted_output):
        ...
        for step in steps:
            if step.get("executor") == "desktop" and step.get("action") == "type_text":
                step.setdefault("payload", {})
                step["payload"]["text"] = execution_task["delivery_payload"].get("send_text", "")
                step["delivery_payload"] = dict(execution_task["delivery_payload"])
    ...
    def simulate_execution(self, execution_task):
        ...
        for step in execution_task.get("execution_steps", []) or []:
            ...
            if executor in ("system", "tool"):
                ...
                if step.get("delivery_payload") is not None:
                    log_item["delivery_payload"] = dict(step.get("delivery_payload") or {})
                logs.append(log_item)

        execution_task["execution_status"] = "done"

        return {
            "desktop_delivery_payload": dict(execution_task.get("delivery_payload") or {}),
