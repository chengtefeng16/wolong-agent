...
def main():
    ...
    summary = {
        ...
        "execution_task": {
            ...
            "desktop_text_payload": _find_desktop_text_payload(safe_get(data, "execution_task", "execution_steps")),
        },
        "execution_result": {
            "execution_status": safe_get(data, "execution_result", "execution_status"),
            "desktop_delivery_payload": _find_desktop_delivery_payload(safe_get(data, "execution_result", "execution_logs")),
        },
        "platform_governance": {
...
def _find_desktop_text_payload(steps):
...
def _find_desktop_delivery_payload(logs):
    if not isinstance(logs, list):
        return None
    for item in logs:
        if item.get("step") == "5_desktop":
            return item.get("delivery_payload")
    return None
