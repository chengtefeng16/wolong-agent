...
async function loadRuntimeCustomers() {
  const indexData = await fetchJson('/runtime/whatsapp/conversation_index.json')
  ...
  if (!items.length) {
    ...
    if (dashboardData?.customers && Array.isArray(dashboardData.customers)) {
      return {
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    const conversation = phone
      ? await fetchJson(`/runtime/whatsapp/conversations/${encodeURIComponent(phone)}.json`)
      : null
    customers.push(mapIndexItemToCustomer(item, conversation, i))
  }
  ...
  return {
    ...
    stats: [
      { label: '准车商', value: statsMap['准车商'] },
      { label: '疑似车商', value: statsMap['疑似车商'] },
      { label: '个人客户', value: statsMap['个人客户'] },
      { label: '沟通无效', value: statsMap['沟通无效'] },
    ],
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    async function loadAll() {
      const runtimeResult = await loadRuntimeCustomers()
      setCustomers(runtimeResult.customers)
      setStats(runtimeResult.stats)
      setSelectedId((prev) => prev || runtimeResult.customers?.[0]?.id || fallbackCustomers[0].id)
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr', gap: '10px', alignItems: 'stretch' }}>
          ...
          <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '10px', background: '#fafafa' }}>
            ...
            <div style={{ fontSize: '12px', lineHeight: 1.8 }}>
              ...
              <div>当前策略：{ingressMode === 'readonly' ? '只观察不扰动' : '其他模式'}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '12px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '12px' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ fontSize: '13px', lineHeight: 1.7, color: '#111827' }}>{selected.reason}</div>
          </div>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '10px', maxHeight: '180px', overflowY: 'auto', background: '#fafafa' }}>
              ...
              )) : <div style={{ fontSize: '12px', color: '#9ca3af' }}>暂无时间线</div>}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', padding: '10px', maxHeight: '260px', overflowY: 'auto', background: '#fafafa' }}>
              ...
              )) : <div style={{ fontSize: '12px', color: '#9ca3af' }}>暂无聊天内容</div>}
            </div>
          </div>
        </div>
      </div>
