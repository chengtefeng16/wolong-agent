...
FRONTEND_RUNTIME_VIEWS_DIR = BASE_DIR.parent / "wolong_h5_console" / "public" / "runtime" / "views"
FRONTEND_WHATSAPP_CONTROL_SNAPSHOT_PATH = FRONTEND_RUNTIME_VIEWS_DIR / "whatsapp_control_snapshot.json"
...
class H5DelegationConfigServiceV2:
    ...
    def _load_existing_active_config(self):
    ...
    def _resolve_whatsapp_control(self, payload: dict):
        default_cfg = self._default_whatsapp_control()
        existing = self._load_existing_active_config()
        existing_cfg = existing.get("whatsapp_control", {}) or {}
        payload_cfg = payload.get("whatsapp_control")

        default_cfg.update(existing_cfg)

        if isinstance(payload_cfg, dict):
            default_cfg.update(payload_cfg)

        return default_cfg

    def _sync_frontend_whatsapp_control_snapshot(self, whatsapp_control: dict):
        snapshot = self._default_whatsapp_control()
        snapshot.update(whatsapp_control or {})
        self._write_json(FRONTEND_WHATSAPP_CONTROL_SNAPSHOT_PATH, snapshot)
        return str(FRONTEND_WHATSAPP_CONTROL_SNAPSHOT_PATH)
    ...
    def save_config(self, payload: dict):
        ...
        whatsapp_control = self._resolve_whatsapp_control(payload)
        ...
        frontend_snapshot_path = self._sync_frontend_whatsapp_control_snapshot(whatsapp_control)
